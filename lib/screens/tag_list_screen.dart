...
/// 标签筛选列表页。
///
/// 接收 [tag] 字符串与可选的 [includeTypes]（多个类型以英文逗号分隔，如
/// `Movie,Series`），网格展示同标签下的内容。点击任一卡片进入对应详情页。
class TagListScreen extends ConsumerWidget {
  ...
  /// 可选的 `IncludeItemTypes`，多个类型以英文逗号分隔。
  final String? includeTypes;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final itemsAsync = ref.watch(
      itemsByTagProvider(TagFilterQuery(tag, includeTypes: includeTypes)),
    );
            ...
            error: (error, _) => SliverFillRemaining(
              child: _ErrorView(
                ...
                onRetry: () => ref.invalidate(
                  itemsByTagProvider(
                    TagFilterQuery(tag, includeTypes: includeTypes),
                  ),
                ),
              ),
            ),
